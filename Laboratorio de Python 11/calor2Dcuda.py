...
def evolucion(u,n_0,n_1,udx2_0,udx2_1,dt,kd,i):
    jp1 = i + n_0
    jm1 = 1 - n_0
    laplaciano = (u[i-1]-2.0*u[i]+u[i+1])*udx2_0 + (u[jm1]-2.0*u[i]+u[i]+u[jp1])*udx2_1
...
@cuda.jit
def solucion_kernal(u_d,un_d,udx2_0,udx2_1,dt,n_0,n_1,kd):
    ii, jj = cuda.grid(2)
    i = ii + n_0*jj
...
# Pasar arreglos de GPU
u_d = cuda.to_device(u)
un_d = cuda.to_device(un)
# Integrar en el tiempo
for t in range(1, pasos+1):
    solucion_kernal[griddim,blockdim](u_d,un_d,udx2[0],udx2[1],dt,n[0],n[1],kd)
